...
data.orthologues.single.copy <- data.orthologues[rowSums(as.matrix(data.orthologues.counts == 1)) == dim(data.orthologues.counts)[2], ]
...
for(i in 1:dim(data.orthologues)[1]){
  for(j in 1:dim(data.orthologues)[2]){
    data.orthologues.single.copy.genes[i, j] <- gsub('', '', data.orthologues.single.copy[i, j])
