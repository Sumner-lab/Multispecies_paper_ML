...
# mischocyttarus is foreground in this test
...
mv input/${species}-${orthogroup}_omegaNotFixed.ctl input/mis-${orthogroup}_omegaNotFixed.ctl

# run algorithm for alternative model (fix_omega = 0 (not fixed))
/lustre/home/ucfaeef/programs/paml4.8/bin/codeml /lustre/home/ucfaeef/projects/multispecies-dnds/input/mis-${orthogroup}_omegaNotFixed.ctl

# check result: resultOnScratch/codeml/branch-site-models/${species}-${orthogroup}-omegaNotFixed
# remove working copy (keeping things tidy)
rm -rf input/mis-${orthogroup}_omegaNotFixed.ctl
