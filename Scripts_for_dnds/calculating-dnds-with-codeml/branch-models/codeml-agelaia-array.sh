...
# agelaia is foreground in model 2
species="agelaia"

# create config file for null model (m0, dn/ds ratio without variable between lineages)
cp input/config-files/template_M0.ctl input/config-files/${orthogroup}_M0.ctl

# update the orthogroup name
sed --in-place "s/OG/${orthogroup}/g" input/config-files/${orthogroup}_M0.ctl

# run algorithm for model 0 (need full path of confid file)
/lustre/home/ucfaeef/programs/paml4.8/bin/codeml /lustre/home/ucfaeef/projects/multispecies-dnds/input/config-files/${orthogroup}_M0.ctl

# create config file for model 2 (m2, dn/ds ratio with the species in foreground branch)
cp input/config-files/${species}-template_M2.ctl input/config-files/${species}-${orthogroup}_M2.ctl

# update the orthogroup name
sed --in-place "s/OG/${orthogroup}/g" input/config-files/${species}-${orthogroup}_M2.ctl

# run algorithm for model 2 (tree species if different)
/lustre/home/ucfaeef/programs/paml4.8/bin/codeml /lustre/home/ucfaeef/projects/multispecies-dnds/input/config-files/${species}-${orthogroup}_M2.ctl
